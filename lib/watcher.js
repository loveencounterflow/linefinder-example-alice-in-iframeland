(function() {
  'use strict';
  var $, Async_pipeline, FS, G, GUY, My_watcher, PATH, Pipeline, after, alert, create_pipeline, debug, defer, demo, echo, file_dt, help, info, inspect, log, plain, praise, rpr, sleep, urge, warn, whisper, xxx, xxx_count;
  ...
  xxx = {
    ...
    $log_all: function() {
      return (d) => {
        return whisper(GUY.datetime.now(), '^345-1^', 'pipeline', d);
      };
    },
    //---------------------------------------------------------------------------------------------------------
    ...
    $html_from_md: function() {
      return async(d, send) => {
        ...
        GUY.temp.with_directory({
          ...
          var error, message, ref, ref1, tmp_path, xxx_count;
  ...
  My_watcher = class My_watcher extends GUY.watch.Watcher {
    ...
    constructor(pipeline) {
      ...
      this._watcher.on('all', (async(path, stats) => {
        return (await this.on_all(path));
      }));
